...
function show($pageNum = 1, $pageSize = 3){
    ...
    $sql_select = "SELECT ImageID,Title,Description,PATH,CityCode,Content FROM temp_myfavor1 limit " . (($pageNum - 1) * $pageSize) . "," . $pageSize;
...
{
    $conn = mysqli_connect('localhost', 'root', 'a45224522.', 'pj2_user'); //准备SQL语句
    $sql_select = "SELECT count(*) num FROM temp_myfavor1";
    $ret = mysqli_query($conn, $sql_select);
...
<head>
    <meta charset="UTF-8">
    <title>Title</title>
    <link rel="stylesheet" type="text/css" href="../CSS/My_Favor.css">
...
<div class="content">
    ...
    foreach ($array as $key => $values) {
        ...
        $form= '<form method=post action="My_Favor_delete.php?ImageID='.$values->ImageID.'">';
        echo $form;
...
<div id="navDiv" class="page">
    ...
    <a class="page_first"
       href="My_Favor.php?pageNum=1">首页</a>
    <a class="page_prev"
       href="My_Favor.php?pageNum=<?php echo $pageNum == 1 ? 1 : ($pageNum - 1) ?>">上一页</a>
    <?php
    echo '<a class="a" class="page_current" href="My_Favor.php?pageNum=1">1</a> ';

    for ($i = 2; $i <= $endPage; $i++) {
        echo '<a  class="a" class="page_other" href="My_Favor.php?pageNum=' . $i . '">' . $i . '</a> ';
    }
    ?>
    <a class="page_next"
       href="My_Favor.php?pageNum=<?php echo $pageNum == $endPage ? $endPage : ($pageNum + 1) ?>">下一页</a>
    <a class="page_last"
       href="My_Favor.php?pageNum=<?php echo $endPage ?>">尾页</a>
</div>
<div class="footer">
